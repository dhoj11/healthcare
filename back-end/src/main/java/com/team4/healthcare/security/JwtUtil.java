package com.team4.healthcare.security;
...
public class JwtUtil {
    ...
    public static String createToken(String staff_id) {
		String token = null;
		try {
			/*
			token = Jwts.builder()
			...
			*/
			...
			builder.claim("staff_id", staff_id); //토큰에 저장되는 데이터
	...
	public static String getStaff_id(String token) {
		...
		try {
			...
			*/
			...
			staff_id = claims.get("staff_id", String.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return staff_id;
	...
	public static void main(String[] args) {
		...
		if(validateToken(jwt)) {
			String staff_id = getStaff_id(jwt);
